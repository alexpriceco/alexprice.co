...
?>
		...
		<em class="comment-awaiting-moderation"><?php _e( 'Hang tight while we approve your comment.' ) ?></em>
		<br />
		<?php endif; ?>

		<div class="comment-meta commentmetadata"><div class="comment-date">
			<?php
				/* translators: 1: date, 2: time */
				printf( __( '%1$s at %2$s' ), get_comment_date(),  get_comment_time() ); ?></div><?php edit_comment_link( __( '(Edit)' ), '&nbsp;&nbsp;', '' );
			?>
		</div>
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$fields   =  array(
		'author' => '<input id="author" placeholder="Name" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /><div class="half-spacer">&nbsp;</div>',
		'email'  => '<input id="email" placeholder="Email" name="email" ' . ( $html5 ? 'type="email"' : 'type="text"' ) . ' value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30" aria-describedby="email-notes"' . $aria_req . ' />'
	);
	...
	$defaults = array(
		'fields'               => $fields,
		'comment_field'        => '<p class="comment-form-comment"><textarea id="comment" placeholder="Comment" name="comment" cols="45" rows="2" aria-describedby="form-allowed-tags" aria-required="true"></textarea></p>',
